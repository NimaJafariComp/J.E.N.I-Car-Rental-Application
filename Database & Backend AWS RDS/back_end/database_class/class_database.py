...
class test:
    ...
    def connect_to_sql_server(self):
        connection_string = (
            ...
            f"SERVER=jenni-database.czg446yqs8iz.us-west-1.rds.amazonaws.com;"
            f"DATABASE=Jenni-Database;"
            f"UID={self.username};"
            f"PWD={self.password};"
            f"PORT=1433;"
        )
        self.mydb = pyodbc.connect(connection_string)
    ...
    def add_car(self, uVIN, uMileage, uMPG, uPrice, uIsActive, uLicensePlate):
        ...
        sql_insert_vehicle = "insert into Vehicles (VIN, Mileage, MPG, Price, IsActive, LicensePlate) values (?, ?, ?, ?, ?, ?)"
    ...
    def update_mileage(self, CarID, updated_mileage):
        ...
        sql_update_mileage = "update Vehicles set Mileage = ? where CarID = ?"
